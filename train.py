...
image_dir = os.path.join(root,'outputs/saved_images')
os.makedirs(image_dir, exist_ok=True)
    
def save_decoded_image(img, name):
    img = img.view(img.size(0), 3, 256, 448)
    save_image(img, name)
...
# gauss_blur = os.listdir('/content/blur_data/')
gauss_blur = os.listdir(os.path.join(root,'blur_data'))
gauss_blur.sort()
sharp = os.listdir(os.path.join(root,'clear_data'))
sharp.sort()
...
class DeblurDataset(Dataset):
    ...
    def __getitem__(self, i):
        blur_path=os.path.join(root,'blur_data')
        ...
        if self.y is not None:
            sharp_path=os.path.join(root,'clear_data')
    ...
    with torch.no_grad():
        for i, data in tqdm(enumerate(dataloader), total=int(len(val_data)/dataloader.batch_size)):
            ...
            if epoch == 0 and i == (len(val_data)/dataloader.batch_size)-1:
                save_decoded_image(sharp_image.cpu().data, name=f"{os.path.join(root,'outputs')}/saved_images/sharp{epoch}.jpg")
                save_decoded_image(blur_image.cpu().data, name=f"{os.path.join(root,'outputs')}/saved_images/blur{epoch}.jpg")
